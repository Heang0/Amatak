...
export default function StoreSidebarMenu({
  ...
  // 2. 💻 MODERN TECH (minimal-tech)
  if (themeStyle === 'minimal-tech') {
    return (
      <div
        className={`fixed inset-0 z-[100] flex flex-col transform transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] bg-[#050B14] ${
          isOpen ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0'
        }`}
      >
        <div className="flex justify-between items-center p-6 border-b border-cyan-500/20 bg-[#0A101C]">
          {storeLogo ? (
            <img src={storeLogo.replace('/upload/', '/upload/w_200,c_limit,q_auto/')} alt={storeName} className="h-8 w-auto object-contain" />
          ) : (
            <span className="text-xl font-mono font-bold text-cyan-400 tracking-wider" style={{ color: primaryColor || '#22d3ee' }}>
              {storeName}
            </span>
          )}
          <button onClick={onClose} className="p-2 text-cyan-500 hover:text-cyan-300 hover:bg-cyan-900/30 rounded-md transition-all" style={{ color: primaryColor || '#22d3ee' }}>
            <X size={24} />
          </button>
        </div>

        <nav className="flex-1 overflow-y-auto flex flex-col p-8 space-y-6">
          <div className="text-[10px] font-mono uppercase tracking-[0.2em] mb-2" style={{ color: primaryColor || '#0891b2' }}>{isKm ? 'ម៉ឺនុយប្រព័ន្ធ' : 'SYSTEM MENU'}</div>
          {[
            { label: homeLabel, href: homeHref },
            { label: productsLabel, href: appendParams(`${basePath}/products`) },
            { label: promotionsLabel, href: appendParams(`${basePath}/promotions`) },
          ].map((item, idx) => (
            <Link key={idx} href={item.href} onClick={onClose} className="group flex items-center justify-between text-2xl font-mono font-light text-gray-300 transition-colors">
              <span className="group-hover:text-white transition-colors">{item.label}</span>
              <span className="opacity-0 group-hover:opacity-100 font-mono text-xs border px-2 py-1 rounded transition-all" style={{ color: primaryColor || '#06b6d4', borderColor: primaryColor || '#06b6d4' }}>GO</span>
            </Link>
          ))}

          {categories.length > 0 && (
            <div className="flex flex-col space-y-4 pt-4 border-t border-cyan-900/30">
              <button onClick={() => setIsCategoriesOpen(!isCategoriesOpen)} className="flex items-center justify-between text-2xl font-mono font-light text-gray-300 hover:text-white transition-colors w-full text-left">
                <span>{allCategoriesLabel}</span>
                <ChevronDown size={24} className={`transition-transform duration-300 ${isCategoriesOpen ? 'rotate-180' : ''}`} style={isCategoriesOpen ? { color: primaryColor || '#22d3ee' } : {}} />
              </button>
              <div className={`flex flex-col pl-4 border-l-2 overflow-hidden transition-all duration-300 ${isCategoriesOpen ? 'max-h-[500px] opacity-100 mt-4 space-y-4' : 'max-h-0 opacity-0'}`} style={{ borderColor: primaryColor || '#083344' }}>
                {categories.filter(c => !c.parentCategory).map(mainCat => (
                  <Link key={mainCat._id} href={appendParams(`${basePath}/category/${mainCat.slug}`)} onClick={onClose} className="text-lg font-mono text-gray-500 hover:text-white transition-colors">
                    {isKm && mainCat.nameKm ? mainCat.nameKm : mainCat.name}
                  </Link>
                ))}
              </div>
          ...
          <div className="mt-auto pt-8 flex flex-col space-y-6 border-t border-cyan-900/30">
             <Link href={cartHref} onClick={onClose} className="flex items-center gap-4 text-sm font-mono text-gray-400 hover:text-white uppercase tracking-widest transition-colors">
              <ShoppingBag size={18} style={{ color: primaryColor || '#22d3ee' }} /> {cartLabel}
            </Link>
            <Link href={favoritesHref} onClick={onClose} className="flex items-center gap-4 text-sm font-mono text-gray-400 hover:text-white uppercase tracking-widest transition-colors">
              <Bookmark size={18} style={{ color: primaryColor || '#22d3ee' }} /> {favoritesLabel}
            </Link>
            <Link href={profileHref} onClick={onClose} className="flex items-center gap-4 text-sm font-mono text-gray-400 hover:text-white uppercase tracking-widest transition-colors">
              <User size={18} style={{ color: primaryColor || '#22d3ee' }} /> {accountLabel}
            </Link>
          </div>
        </nav>

        <div className="p-6 border-t border-cyan-500/20 bg-[#0A101C] flex justify-between items-center">
          <Link href={langHref} className="opacity-70 hover:opacity-100 transition-opacity">
            {/* eslint-disable-next-line @next/next/no-img-element */}
