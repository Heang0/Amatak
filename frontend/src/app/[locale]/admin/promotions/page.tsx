...
export default function AdminPromotionsPage({ params }: { params: { locale: string } }) {
 ...
 <div className="space-y-6">
  ...
  <div>
   ...
   <p className="text-sm text-gray-600 dark:text-gray-400 mt-0.5">{isKm ? 'បង្កើតលេខកូដបញ្ចុះតម្លៃសម្រាប់អតិថិជនរបស់អ្នក។' : 'Create discount codes for your customers.'}</p>
  </div>
  <button 
  ...
  <div className="bg-white dark:bg-[#121212] dark:border dark:border-white/10 rounded-none border border-none border-none overflow-x-auto">
  <table className="w-full text-left border-collapse">
   ...
   {promos.length === 0 ? (
    ...
    <td className="p-4">
     ...
     className={`flex items-center gap-1.5 px-2.5 py-1 rounded-none text-xs font-bold ${ promo.isActive ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-600 dark:text-gray-400 dark:bg-gray-800 ' }`}
     >
    ...
    <td className="p-4 text-right">
     ...
     <button onClick={() => handleEditPromo(promo)} className="p-1.5 text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-none transition-colors">
      <Edit2 size={16} />
     </button>
     <button onClick={() => deletePromo(promo._id)} className="p-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-none transition-colors">
      <Trash2 size={16} />
     </button>
     </div>
    </td>
  ...
  <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50">
   ...
   <form onSubmit={handleSubmit} className="p-6 space-y-4">
    {error && <div className="p-3 bg-red-50 text-red-600 rounded-none text-sm">{error}</div>}
    ...
    <input type="text" value={code} onChange={e => setCode(e.target.value)} required className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-none focus:outline-none focus:ring-2 focus:ring-black dark:bg-gray-900 uppercase" />
    </div>
    ...
    <div>
     ...
     <select value={discountType} onChange={e => setDiscountType(e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-none focus:outline-none focus:ring-2 focus:ring-black dark:bg-gray-900">
    ...
    <div>
     ...
     <input type="number" step="0.01" min="0" value={discountValue} onChange={e => setDiscountValue(e.target.value)} required className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-none focus:outline-none focus:ring-2 focus:ring-black dark:bg-gray-900" />
    </div>
    </div>
    ...
    <div>
     ...
     <input type="number" step="0.01" min="0" value={minPurchase} onChange={e => setMinPurchase(e.target.value)} placeholder={isKm ? '0 សម្រាប់មិនមានកំណត់' : '0 for no min'} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-none focus:outline-none focus:ring-2 focus:ring-black dark:bg-gray-900" />
    </div>
    <div>
    ...
    <input type="date" value={validUntil} onChange={e => setValidUntil(e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-none focus:outline-none focus:ring-2 focus:ring-black dark:bg-gray-900" />
    </div>

    <button type="submit" className="w-full py-2.5 bg-[#E84C3D] hover:bg-red-600 text-white rounded-none font-bold transition-all shadow-md shadow-red-500/20 mt-4">
