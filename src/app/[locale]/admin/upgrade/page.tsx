...
export default function UpgradePlan() {
 ...
 <div className="space-y-6">
  ...
  <div>
   ...
   <p className="text-sm text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] /40 mt-0.5">Manage your subscription plan</p>
  </div>
  ...
  <div className="flex items-center bg-gray-100 dark:bg-white/[0.06] p-1 rounded-lg shrink-0">
   ...
   className={`px-5 py-1.5 rounded-md text-sm font-semibold transition-all ${billingCycle === 'monthly' ? 'bg-white dark:bg-white text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa] shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none' : 'text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] /40 hover:text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa] dark:hover:text-white'}`}
   >
   {isKm ? 'ប្រចាំខែ' : 'Monthly'}
   </button>
   <button 
   onClick={() => setBillingCycle('annually')}
   className={`px-5 py-1.5 rounded-md text-sm font-semibold transition-all flex items-center gap-2 ${billingCycle === 'annually' ? 'bg-white dark:bg-white text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa] shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none' : 'text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] /40 hover:text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa] dark:hover:text-white'}`}
   >
  ...
  {loading ? (
  <p className="text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa]">{t('loading')}</p>
  ) : (
  <>
   {/* Current Subscription Overview */}
   {storeData && currentStorePlan && (
   <div className="bg-white dark:bg-[#121212] dark:border dark:border-white/10 border-none rounded-[20px] p-6 shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
    <div>
    <h3 className="text-sm font-semibold text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] uppercase tracking-wider mb-1">
     {isKm ? 'គម្រោងបច្ចុប្បន្នរបស់អ្នក' : 'Your Current Plan'}
    </h3>
    <div className="flex items-center gap-3">
     <span className="text-2xl font-bold text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa]">
    ...
    <p className="text-sm text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] mt-2">
    ...
    <div className="flex items-center gap-4 bg-[#F4F7FE] dark:bg-[#080808] p-4 rounded-[20px] border-none">
    ...
    <div>
     <p className="font-semibold text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa]">{storeData.name}</p>
     <p className="text-xs text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa]">{storeData.slug}</p>
    </div>
    </div>
   </div>
   )}
   ...
   <div 
    key={plan._id} 
    className={`bg-white dark:bg-[#121212] dark:border dark:border-white/10 p-8 rounded-[20px] shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none border flex flex-col transition-all relative ${ currentPlanId === plan._id ? (isExpired() ? 'border-orange-500 ring-1 ring-orange-500' : 'border-[#E84C3D] ring-1 ring-[#E84C3D] shadow-md scale-[1.02]') : 'border-none border-none hover:border-red-200 dark:hover:border-red-900/50 hover:scale-[1.01]' }`}
   >
    <h3 className="text-xl font-bold text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa] flex items-center justify-between">
    ...
    <div className="flex items-baseline gap-2">
     <span className="text-4xl font-extrabold text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa]">
     ${getDisplayPrice(plan)}
     </span>
     <span className="text-sm font-medium text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa]">
     /{billingCycle === 'annually' ? (isKm ? 'ឆ្នាំ' : 'yr') : (isKm ? 'ខែ' : 'mo')}
     </span>
    </div>
    {getOriginalPrice(plan) && (
     <div className="text-sm font-medium text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] line-through mt-1">
     ${getOriginalPrice(plan)}/{isKm ? 'ឆ្នាំ' : 'yr'}
     </div>
    )}
    </div>
    <ul className="mt-6 space-y-4 flex-1 text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa]">
    ...
    return (
     ...
     disabled={isDisabled}
     className={`mt-8 block w-full font-semibold py-3 px-4 rounded-lg text-center transition-colors shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none ${ isDisabled ? 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] cursor-not-allowed' : 'bg-gray-900 dark:bg-white text-white hover:bg-gray-800 dark:hover:bg-gray-100' }`}
     >
     {currentPlanId === plan._id 
    ...
    <div className={`mt-3 text-xs text-center font-medium ${isExpired() ? 'text-red-500' : 'text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa]'}`}>
  ...
  {!loading && (
  <div className="mt-12 max-w-2xl mx-auto border-t border-none border-none pt-10">
   <div className="mb-6">
   <h3 className="text-xl font-bold text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa]">{isKm ? 'វិធីសាស្ត្រទូទាត់' : 'Payment Method'}</h3>
   <p className="text-sm text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] mt-1">{isKm ? 'វិធីសាស្ត្រដែលត្រូវបានទទួលយក' : 'Accepted payment methods'}</p>
   </div>

   <div className="bg-white dark:bg-[#121212] dark:border dark:border-white/10 border border-none border-none rounded-[20px] p-4 md:p-5 shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none transition-all cursor-pointer relative overflow-hidden flex items-center justify-between group hover:border-[#E1232E]/30 hover:shadow-md">
   
   <div className="flex items-center gap-5">
    {/* Logo Box */}
    <div className="w-12 h-12 rounded-[20px] bg-[#E1232E] flex items-center justify-center shrink-0 shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none overflow-hidden">
    <img 
    ...
    <div className="flex items-center gap-2 mb-1">
     ...
     <span className="text-[10px] text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] font-medium hidden sm:inline-block">• {isKm ? 'អនុញ្ញាតភ្លាមៗ' : 'Instant Approval'}</span>
    </div>
    <h4 className="text-base font-bold text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa] uppercase">Bakong KHQR</h4>
    <p className="text-xs text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] font-medium">{isKm ? 'ការទូទាត់តាម Bakong' : 'Payment via Bakong KHQR'}</p>
    </div>
   </div>

   {/* Selected Indicator */}
   <div className="w-6 h-6 rounded-full bg-[#E1232E] flex items-center justify-center shrink-0 shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none mr-2">
