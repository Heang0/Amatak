...
export default function AdminCategories() {
  ...
  const renderCategoryRow = (category: Category, isSub: boolean = false) => {
    ...
    return (
      <tr key={category._id} className={`border-b border-gray-100 dark:border-gray-800 transition-colors ${isSub ? 'bg-gray-50/50 dark:bg-[#111C44]/20' : 'hover:bg-gray-50 dark:hover:bg-[#111C44]/50'}`}>
          ...
          <div className={`flex items-center ${isSub ? 'ml-8' : ''}`}>
            ...
            {isEditing ? (
              <div className="flex flex-col gap-2 w-full max-w-sm">
                <div className="flex gap-2">
                  <input
                    ...
                    className="w-full px-3 py-1.5 text-sm border-none rounded bg-[#F4F7FE] dark:bg-[#0B1437] text-gray-900 dark:text-white"
                    autoFocus
                  />
                  <input
                    type="text"
              ...
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${isSub ? 'bg-gray-100 dark:bg-gray-800 text-gray-500' : 'bg-[#F4F7FE] dark:bg-[#0B1437] text-[#2B3674] dark:text-[#A3AED0]'}`}>
                  <Tag size={14} />
                </div>
                <div>
                  <p className="font-bold text-[#2B3674] dark:text-white text-sm">
                    ...
                    {getSecondaryCategoryName(category) && (
                      <span className="text-[#A3AED0] font-normal ml-2 text-xs">{getSecondaryCategoryName(category)}</span>
                    )}
                  </p>
                  <p className="text-[11px] text-[#A3AED0] font-mono mt-0.5">/{category.slug}</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-end gap-2">
            {editingCategory?._id !== category._id && (
              <button
                ...
                className="p-2 text-[#A3AED0] hover:text-[#2B3674] dark:hover:text-white transition-colors rounded-none"
                title={t('edit')}
              >
                <Edit2 size={16} />
              </button>
            ...
            <button
               ...
               className="p-2 text-[#A3AED0] hover:text-red-600 transition-colors rounded-none"
               title={t('delete')}
             >
               <Trash2 size={16} />
             </button>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 pb-2">
        <div>
          <h2 className="text-xl font-bold text-[#2B3674] dark:text-white">{t('title')}</h2>
          <p className="text-sm text-[#A3AED0] mt-1">{locale === 'km' ? 'គ្រប់គ្រងប្រភេទផលិតផលរបស់អ្នក' : 'Manage your product categories'}</p>
        </div>
        <button
      ...
      {loading ? (
        <div className="bg-white dark:bg-[#111111] rounded-2xl border-none p-12 flex flex-col items-center gap-3 shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none">
          <div className="w-8 h-8 border-2 border-[#2B3674] dark:border-white border-t-transparent rounded-full animate-spin" />
        </div>
      ) : categories.length === 0 ? (
        <div className="bg-white dark:bg-[#111111] rounded-2xl border-none p-16 text-center shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none">
          <div className="w-16 h-16 bg-[#F4F7FE] dark:bg-[#0B1437] rounded-full flex items-center justify-center mx-auto mb-4">
            <Tag className="w-8 h-8 text-[#2B3674] dark:text-[#A3AED0]" />
          </div>
          <h3 className="text-lg font-bold text-[#2B3674] dark:text-white mb-2">{t('no_categories')}</h3>
          <p className="text-[#A3AED0] text-sm mb-6 max-w-sm mx-auto">{t('no_categories_desc')}</p>
          <button
            onClick={() => setIsAddModalOpen(true)}
        ...
        <div className="bg-white dark:bg-[#111111] rounded-2xl shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none border-none overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-800 bg-[#F4F7FE]/50 dark:bg-[#0B1437]/30">
                  <th className="px-6 py-4 text-left text-[11px] font-bold text-[#A3AED0] uppercase tracking-widest">{t('title')}</th>
                  <th className="px-6 py-4 text-right text-[11px] font-bold text-[#A3AED0] uppercase tracking-widest">{locale === 'km' ? 'សកម្មភាព' : 'Action'}</th>
                </tr>
              </thead>
      ...
      {isAddModalOpen && typeof window !== 'undefined' && createPortal(
        <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 ${locale === 'km' ? 'font-khmer' : ''}`}>
          <div className="bg-white dark:bg-[#111111] border-none rounded-2xl max-w-md w-full shadow-xl relative overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-gray-100 dark:border-gray-800">
              <h3 className="text-lg font-bold text-[#2B3674] dark:text-white">{t('create_new')}</h3>
              <button onClick={() => setIsAddModalOpen(false)} className="text-[#A3AED0] hover:text-[#2B3674] dark:hover:text-white transition-colors rounded-none">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateCategory} className="p-6">
              ...
              <div className="flex gap-2 mb-6 bg-[#F4F7FE] dark:bg-[#0B1437] p-1 rounded-lg w-full">
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className={`flex-1 py-1.5 text-sm font-bold rounded-md transition-colors ${categoryType === 'main' ? 'bg-white dark:bg-[#111111] text-[#2B3674] dark:text-white shadow-sm' : 'text-[#A3AED0] hover:text-[#2B3674] dark:hover:text-white'}`}
                >
                ...
                <button
                  ...
                  className={`flex-1 py-1.5 text-sm font-bold rounded-md transition-colors ${categoryType === 'sub' ? 'bg-white dark:bg-[#111111] text-[#2B3674] dark:text-white shadow-sm' : 'text-[#A3AED0] hover:text-[#2B3674] dark:hover:text-white'}`}
                >
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between mb-1">
                    <label className="text-sm font-bold text-[#2B3674] dark:text-white">Name (EN)</label>
                  </div>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full px-4 py-3 border-none rounded-xl bg-[#F4F7FE] dark:bg-[#0B1437] text-[#2B3674] dark:text-white focus:ring-2 focus:ring-[#2B3674] dark:focus:ring-white outline-none"
                  />
                </div>
                
                <div>
                  <div className="flex justify-between mb-1">
                    <label className="text-sm font-bold text-[#2B3674] dark:text-white">ឈ្មោះ (KM)</label>
                  </div>
                  <input
                    type="text"
                ...
                {categoryType === 'sub' && (
                  <div>
                    <label className="block text-sm font-bold text-[#2B3674] dark:text-white mb-1">Parent Category</label>
                    <select
                      required
              ...
              <div className="mt-8 flex gap-3">
                <button
                  ...
                  className="flex-1 py-3 text-sm font-bold text-[#2B3674] dark:text-white bg-[#F4F7FE] dark:bg-[#0B1437] rounded-none transition-colors hover:bg-gray-200 dark:hover:bg-gray-800"
                >
                  {t('cancel')}
                </button>
                <button
      ...
      {deleteModal.isOpen && typeof window !== 'undefined' && createPortal(
        <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 ${locale === 'km' ? 'font-khmer' : ''}`}>
          <div className="bg-white dark:bg-[#111111] border-none rounded-2xl max-w-md w-full shadow-2xl p-6 relative overflow-hidden">
            {deleteModal.hasProductsError ? (
              ...
              <div className="text-center">
                ...
                <div className="flex gap-3">
                  <button onClick={() => setDeleteModal(prev => ({ ...prev, isOpen: false }))} className="flex-1 py-2.5 bg-gray-100 dark:bg-[#0B1437] text-gray-700 dark:text-white rounded-none font-bold text-sm">Cancel</button>
