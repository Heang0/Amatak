...
export default function ThemeCustomizer() {
 ...
 <>
  ...
  <div className="flex items-center gap-3 md:gap-4">
   ...
   <h2 className="text-xl md:text-3xl font-bold text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa] flex items-center gap-2">
  ...
  <div className="lg:col-span-4 space-y-6">
   <div className="bg-white dark:bg-[#121212] dark:border dark:border-white/10 p-6 rounded-[20px] border-none shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none">
   <h3 className="text-lg font-bold text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa] mb-4">{t('color_title')}</h3>
   <p className="text-sm text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] mb-4">{t('color_desc')}</p>
   <div className="flex flex-col gap-4">
    ...
    {['#000000', '#111111', '#E84C3D', '#E67E22', '#2ECC71', '#3498DB', '#9B59B6', '#E91E63'].map(color => (
     ...
     className={`w-8 h-8 rounded-full border-2 transition-all ${ primaryColor.toUpperCase() === color ? 'border-gray-900 dark:border-white scale-110 shadow-md' : 'border-transparent hover:scale-105 shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none' }`}
    ...
    <input
     ...
     className="w-16 h-16 rounded-[20px] cursor-pointer border-2 border-none border-none p-0 bg-transparent overflow-hidden"
    />
    <input
     type="text"
     value={primaryColor}
     onChange={(e) => setPrimaryColor(e.target.value)}
     className="px-4 py-2 border-none rounded-lg bg-[#F4F7FE] dark:bg-[#080808] text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa] font-mono uppercase focus:outline-none focus:ring-2 focus:ring-gray-200"
    />
    </div>
   </div>
   </div>
   ...
   <div className="bg-white dark:bg-[#121212] dark:border dark:border-white/10 p-6 rounded-[20px] border-none shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none">
   <h3 className="text-lg font-bold text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa] mb-4">{t('style_title')}</h3>
   <p className="text-sm text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] mb-4">{t('style_desc')}</p>
   <div className="space-y-3">
    ...
    <div 
     ...
     className={`p-4 rounded-[20px] border-2 cursor-pointer transition-all flex items-center justify-between ${ themeStyle === style.id ? 'border-[#E84C3D] bg-red-50 dark:bg-red-900/10' : 'border-none border-none hover:border-gray-300 dark:hover:border-gray-600' }`}
    >
     <div>
     <h4 className="font-bold text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa]">{style.name}</h4>
     <p className="text-xs text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] mt-1">{style.desc}</p>
     </div>
     <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${ themeStyle === style.id ? 'border-[#E84C3D]' : 'border-gray-300 dark:border-gray-700' }`}>
  ...
  {/* Live Preview Column */}
  <div className="lg:col-span-8 bg-[#F4F7FE] dark:bg-[#080808] p-4 lg:p-8 rounded-[20px] border border-none border-none flex flex-col items-center justify-start min-h-[500px] h-[600px] md:h-[800px]">
   
   {/* Mock Browser URL Bar */}
   <div className="w-full mb-4 bg-white dark:bg-gray-900 border-none rounded-t-lg shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none flex items-center px-4 py-3 gap-2 sticky top-0 z-10">
   ...
   <div className="w-full flex-1 rounded-b-lg border border-none border-none flex items-center justify-center text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa]">
    {t('loading_preview')}
   </div>
   )}
