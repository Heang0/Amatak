 ...
 return (
  <div className="space-y-6 max-w-5xl mx-auto">
   <div className="flex justify-between items-center">
    <h2 className="text-3xl font-bold text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa]">Stores & Subscriptions</h2>
   </div>

   {loading ? (
    <p className="text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa]">Loading stores...</p>
   ) : (
    <div className="bg-white dark:bg-[#121212] dark:border dark:border-white/10 rounded-[20px] shadow-[0_18px_40px_rgba(112,144,176,0.12)] dark:shadow-none border border-none border-none overflow-hidden">
     <table className="min-w-full divide-y divide-[#F4F7FE] dark:divide-white/10">
      <thead className="bg-[#F4F7FE] dark:bg-[#000000]">
       <tr>
        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] uppercase tracking-wider">Store Name</th>
        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] uppercase tracking-wider">Owner</th>
        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] uppercase tracking-wider">Custom Domain</th>
        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] uppercase tracking-wider">Current Plan</th>
        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] uppercase tracking-wider">Expiry</th>
        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] uppercase tracking-wider">Status</th>
      ...
      <tbody className="divide-y divide-[#F4F7FE] dark:divide-white/10">
       {stores.map((store) => (
        <tr key={store._id} className="hover:bg-[#F4F7FE] dark:hover:bg-gray-800/50 transition-colors">
         ...
         <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-[#a1a1aa] dark:text-[#d4d4d8] font-medium">
          {store.ownerId?.name}
          <div className="text-xs text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] font-normal">{store.ownerId?.email}</div>
         </td>
         <td className="px-6 py-4 whitespace-nowrap text-sm">
          {store.customDomain ? (
           <span className="text-blue-600 dark:text-blue-400 font-medium">{store.customDomain}</span>
          ) : (
           <span className="text-gray-600 dark:text-[#a1a1aa] italic">None</span>
          )}
         </td>
         <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-[#a1a1aa] dark:text-[#d4d4d8]">
       ...
       {stores.length === 0 && (
        <tr>
         <td colSpan={6} className="px-6 py-8 text-center text-sm text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa]">
   ...
   {isDomainModalOpen && selectedStore && (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
     <div className="w-full max-w-md bg-white dark:bg-[#121212] dark:border dark:border-white/10 p-6 rounded-[20px] shadow-xl overflow-hidden border border-none border-none">
      <h2 className="text-xl font-bold text-gray-900 dark:text-[#fafafa] dark:text-[#fafafa] mb-2">Edit Custom Domain</h2>
      <p className="text-sm text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa] mb-6">
       Setting custom domain for <strong>{selectedStore.name}</strong>
      </p>

      <div className="space-y-4">
       <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-[#d4d4d8] mb-2">Domain Name</label>
        <input
         type="text"
        ...
        />
        <p className="mt-2 text-xs text-gray-600 dark:text-[#a1a1aa] dark:text-gray-600 dark:text-[#a1a1aa]">Leave empty to remove the custom domain.</p>
       </div>

       <div className="flex gap-3 pt-4">
        <button
         onClick={() => setIsDomainModalOpen(false)}
         className="flex-1 px-4 py-2.5 text-sm font-semibold text-gray-700 dark:text-[#d4d4d8] bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg transition-colors"
        >
         Cancel
        </button>
        <button
