    ...
    <>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="px-6 py-2.5 bg-[#E84C3D] hover:bg-red-600 text-white font-bold rounded-none transition-colors flex items-center gap-2 shadow-sm"
        >
          <Save size={18} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white dark:bg-[#111111] p-6 rounded-2xl border-none shadow-sm">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">{t('color_title')}</h3>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">{t('color_desc')}</p>
            <div className="flex flex-col gap-4">
              <div className="flex flex-wrap gap-3 mb-2">
                {['#000000', '#111111', '#E84C3D', '#E67E22', '#2ECC71', '#3498DB', '#9B59B6', '#E91E63'].map(color => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full border-2 transition-all ${ primaryColor.toUpperCase() === color ? 'border-gray-900 dark:border-white scale-110 shadow-md' : 'border-transparent hover:scale-105 shadow-sm' }`}
              ...
              <div className="flex items-center gap-4">
                ...
                <input
                  ...
                  value={primaryColor}
                  onChange={(e) => setPrimaryColor(e.target.value)}
                  className="px-4 py-2 border-none rounded-lg bg-[#F4F7FE] dark:bg-[#080808] text-gray-900 dark:text-white font-mono uppercase focus:outline-none focus:ring-2 focus:ring-gray-200"
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-[#111111] p-6 rounded-2xl border-none shadow-sm">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">{t('style_desc')}</p>
            <div className="space-y-3">
              {[
                ...
                <div 
                  ...
                  className={`p-4 rounded-xl border-2 cursor-pointer transition-all flex items-center justify-between ${ themeStyle === style.id ? 'border-[#E84C3D] bg-red-50 dark:bg-red-900/10' : 'border-gray-100 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-600' }`}
                >
                  <div>
                    <h4 className="font-bold text-gray-900 dark:text-white">{style.name}</h4>
                    <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">{style.desc}</p>
                  </div>
        ...
        {/* Live Preview Column */}
        <div className="lg:col-span-8 bg-[#F4F7FE] dark:bg-[#080808] p-4 lg:p-8 rounded-2xl border border-gray-200 dark:border-gray-800 flex flex-col items-center justify-start min-h-[500px] h-[600px] md:h-[800px]">
          
          {/* Mock Browser URL Bar */}
          <div className="w-full mb-4 bg-white dark:bg-gray-900 border-none rounded-t-lg shadow-sm flex items-center px-4 py-3 gap-2 sticky top-0 z-10">
          ...
          {storeSlug ? (
            ...
            <div className="w-full flex-1 rounded-b-lg border border-gray-200 dark:border-gray-800 flex items-center justify-center text-gray-600 dark:text-gray-400">
              {t('loading_preview')}
            </div>
          )}
