...
export default function CartPage({ params }: { params: { slug: string, locale: string } }) {
  ...
  if (themeStyle === 'minimal-tech') {
    if (items.length === 0) {
      return (
        <div className="w-full mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-2 sm:pt-4 pb-20 min-h-[70vh] bg-[#050B14] font-mono">
          <div className="flex items-center justify-between py-3 border-b border-cyan-900/40 mb-6">
            <h1 className="text-sm font-bold text-cyan-400 flex items-center gap-2">
              <span className="uppercase tracking-[0.2em]">{text.shoppingBag}</span>
          ...
          <div className="max-w-md mx-auto py-16 px-6 text-center border border-cyan-900/40 bg-[#0D1322] space-y-6 mt-12">
            ...
            <div>
              <h2 className="text-sm font-bold text-cyan-50 uppercase tracking-[0.1em] mb-2">{text.cartEmpty}</h2>
              <p className="text-xs text-cyan-600/80">{text.cartEmptyDesc}</p>
            </div>
    ...
    return (
      <div className="w-full mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 pt-2 sm:pt-4 pb-20 min-h-[70vh] bg-[#050B14] font-mono">
        <div className="flex items-center justify-between py-4 border-b border-cyan-900/40 mb-8">
          <h1 className="text-sm font-bold text-cyan-400 flex items-center gap-2">
            <span className="uppercase tracking-[0.2em]">{text.shoppingBag}</span>
            <span className="text-cyan-800">| {items.reduce((acc, i) => acc + i.quantity, 0)} |</span>
          </h1>
          <Link href={storeHomeHref} className="text-xs text-cyan-600 hover:text-cyan-300 uppercase tracking-widest transition-colors">
            {text.continueShopping}
          </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          <div className="lg:col-span-8 divide-y divide-cyan-900/30 border-y border-cyan-900/30">
            {items.map((item) => (
              <div key={item.cartItemId} className="py-5 flex gap-5 items-start">
                <div className="w-24 aspect-square bg-[#0D1322] border border-cyan-900/50 shrink-0 p-1">
                  {item.imageUrl ? <img src={item.imageUrl} alt={item.title} className="w-full h-full object-contain" /> : <div className="w-full h-full flex items-center justify-center text-cyan-900"><ShoppingBag size={20} /></div>}
                </div>
                <div className="flex flex-col flex-1 min-w-0">
                  <div className="flex justify-between items-start gap-2">
                    <h3 className="font-bold text-cyan-50 text-sm tracking-wide leading-snug">{isKm && item.titleKm ? item.titleKm : item.title}</h3>
                    <button onClick={() => removeItem(item.cartItemId)} className="p-1 text-cyan-600 hover:text-red-400 transition-colors shrink-0"><X size={16} /></button>
                  </div>
                  {item.selectedVariants && Object.keys(item.selectedVariants).length > 0 && (
                    <div className="text-[10px] text-cyan-500 mt-2 uppercase tracking-[0.2em]">
                      {Object.entries(item.selectedVariants).map(([k, v]) => `${k}: ${v}`).join(' // ')}
                    </div>
                  )}
                  <div className="mt-5 flex items-center justify-between">
                    <div className="flex items-center border border-cyan-900/60 bg-[#0D1322] h-9 px-1.5 shrink-0">
                      <button onClick={() => updateQuantity(item.cartItemId, Math.max(1, item.quantity - 1))} className="p-1 text-cyan-600 hover:text-cyan-300"><Minus size={12} /></button>
                      <span className="w-8 text-center font-bold text-cyan-100 text-xs">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.cartItemId, item.quantity + 1)} className="p-1 text-cyan-600 hover:text-cyan-300"><Plus size={12} /></button>
                    </div>
                    <span className="font-bold text-cyan-400 text-sm tracking-wide">${(item.price * item.quantity).toFixed(2)}</span>
                  </div>
                </div>
              </div>
            ))}
          </div>
          <div className="lg:col-span-4 p-6 border border-cyan-900/40 bg-[#0D1322] shadow-[0_0_20px_rgba(34,211,238,0.05)] space-y-6">
            <h2 className="text-xs font-bold text-cyan-500 uppercase tracking-[0.2em] pb-4 border-b border-cyan-900/40">SYS_SUMMARY</h2>
            <div className="space-y-3 text-xs tracking-wider">
              <div className="flex justify-between text-cyan-600/80"><span>{text.subtotal}</span><span className="font-bold text-cyan-300">${getTotalPrice().toFixed(2)}</span></div>
              <div className="flex justify-between text-cyan-600/80"><span>SHIPPING</span><span className="text-cyan-800">PENDING</span></div>
            </div>
            <div className="pt-4 border-t border-cyan-900/40 flex justify-between items-center">
              <span className="text-xs font-bold text-cyan-400 uppercase tracking-[0.2em]">TOTAL</span>
              <span className="text-xl font-black" style={{ color: primaryColor || '#22d3ee' }}>${getTotalPrice().toFixed(2)}</span>
            </div>
            <button onClick={() => router.push(checkoutHref)} className="w-full py-4 text-black text-xs font-bold uppercase tracking-[0.2em] flex items-center justify-center gap-3 hover:opacity-90 transition-all shadow-[0_0_15px_rgba(34,211,238,0.2)]" style={{ backgroundColor: primaryColor || '#22d3ee' }}>
